    ...
    <>
      ...
      <Stack spacing={3}>
        <Stack direction='row' justifyContent='space-between' alignItems='center' sx={{ p: 3, pb: 0 }}>
          <GlobalFilter preGlobalFilteredRows={preGlobalFilteredRows} globalFilter={globalFilter}
...
const ProductList = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ value }: { value: number }) => <NumberFormat value={value} displayType='text' thousandSeparator
                                                              prefix='$' />
      },
      {
        Header: 'Qty',
      ...
      },
      {
        ...
        Cell: ({ value }: { value: string }) => (
          <Chip color={value ? 'success' : 'error'} label={value ? 'In Stock' : 'Out of Stock'} size='small'
                variant='light' />
        )
      },
      {
        ...
        Cell: ({ row }: { row: Row }) => {
          const collapseIcon = row.isExpanded ?
            <Add style={{ color: theme.palette.error.main, transform: 'rotate(45deg)' }} /> : <Eye />;
          return (
  ...
  const renderRowSubComponent = useCallback(({ row }: { row: Row }) => <ProductView
    data={products[Number(row.id)]} />, [products]);
